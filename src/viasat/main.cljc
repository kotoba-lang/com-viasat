(ns viasat.main
  "Kotodama WASM entrypoint for the Viasat clean-room actor (L4) — Clojure port.
  ...
  facts (`viasat.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "viasat")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"viasat.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "viasat-compat" :tier tier :entities entities} 200])
...
(defn start! [] :viasat-compat/ready)
